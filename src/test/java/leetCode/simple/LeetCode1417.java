...
public class LeetCode1417 {
    ...
    public String reformat(String s) {
        //先把字符串s 拆成两个list,然后判断两个list的size()差是不是1
        ...
        if (Math.abs(numberList.size() - alphaList.size()) >1) {
            ...
            List<Character> smallList = numberList.size() < alphaList.size() ? numberList : alphaList;
            List<Character> bigList = numberList.size() < alphaList.size() ? alphaList : numberList;
