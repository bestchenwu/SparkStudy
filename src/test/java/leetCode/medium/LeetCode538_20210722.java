...
// 树中的所有值 互不相同 。
// 给定的树为二叉搜索树。
public class LeetCode538_20210722 {

    public int helpConvertBST(TreeNode root, int sum){
        if(root == null){
            return sum;
        }
        int rightSum = helpConvertBST(root.right, sum);
        root.val+=rightSum;
        return helpConvertBST(root.left,root.val);
    }
